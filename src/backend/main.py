"""Main module for the Data Analysis Chatbot."""
...
class DataAnalysisBot:
    ...
    def __init__(self):
        ...
        self.dataframe_info: str | None = None

    def load_csv(self, file_content: str, filename: str) -> str:
        ...
        try:
            ...
            self.dataframe_info = (
                ...
                f"Shape: {self.current_dataframe.shape[0]} rows, "
                f"{self.current_dataframe.shape[1]} columns\n"
            ...
            self.conversation_history.append(
                {
                    "role": "system",
                    "content": f"CSV file '{filename}' has been loaded. "
                    f"{self.current_dataframe.shape[0]} rows and "
                    f"{self.current_dataframe.shape[1]} columns.",
                }
            )
    ...
    def chat(self, message: str) -> str:
        ...
        if self.current_dataframe is not None:
            ...
            context = (
                f"The user has loaded a CSV file with the following information: "
                f"\n{self.dataframe_info}\n\n"
            )
            ...
            if any(
                ...
                context += (
                    f"Here's a sample of the data (first 5 rows):\n{sample_data}\n\n"
                )

        # Prepare messages for the LLM
        messages = [
            HumanMessage(
                content=f"{context}User question: {message}\n\n"
                f"Please provide a helpful response about the data."
            )
        ]
